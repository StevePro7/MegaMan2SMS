...
;	ret z
	...
	and %11111000
	jr nz, +
		;no column to load in the next frame
		ld a, (columnToLoad)
		sub LoadColumnLeft ; we want a to be in zero
		ret nz	;no column to load now
	
		inc a	;ld a, NoLoadColumn
		ld (columnToLoad), a
		ld a, l
		add a, 8
		ld l, a
		jp nc,gameplayMapsEngine.loadColumn	;no column to load in the next frame, load now
		jp gameplayMapsEngine.IncHloadColumn
+:		
		ld de, columnToLoad
		ld a, (de)
		cp LoadColumnLeft
		jr z, ++
			...
			jr z, +
				inc a ; ld LoadColumnLeft
				ld (de), a
				ld a, l	;	get the previous position
				or %00000111 ;force to a multiple of eight
				inc a
				ld (visualHScroll), a ;save for the visual since in the next frame we
									 ;load another column, and we can't let artfacts
									;appear
			ret
+:	
	;if there was a right column to load, it means that we are just changing sides, and there is no need
	;to load neither in the left nor in the right, since the left one will be already there		
			inc a
			ld (de), a
		ret
++:		
	...
	add a, 8
	ld l, a
	jp nc,gameplayMapsEngine.loadColumn
	jp gameplayMapsEngine.IncHloadColumn
...
;	ret z
	...
	jr nz, +
		...
		inc a	;ld a, NoLoadColumn
		ld (columnToLoad), a 
		jp gameplayMapsEngine.loadColumnRight	;no column to load in the next frame, load now
		
+:		
		ld de, columnToLoad
		ld a, (de)
		dec a ; check if there is a column to load on the right($00 before dec)
		jp m, +
			ld (de), a	;if there was a left column to load, it means that we are just changing sides, and there is no need
						;to load neither in the left nor in the right, since the right one will be already there
			ret nz
				ld a, l
				and %11111000
				ld (visualHScroll), a; if there is a column to load in the next frame, we need to
									  ;to force the visualHScroll position to a multiple of 8
									  ;to make sure that there will be no artifacts when  loading
									  ;the column
			ret
+:
	ld a, l
